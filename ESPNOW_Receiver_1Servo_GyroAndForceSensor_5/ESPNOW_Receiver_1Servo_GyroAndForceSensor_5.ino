...
void mapAndWriteValues()
{
  ...
  Serial.print("valLeftForce1: ");
  Serial.println(valRightForce1);
  Serial.print("valRightForce2: ");
...
/* ServoA0 THROTTLE (FORWARD- FORCE)*******************************************/
  ...
  // Full Swing
  if ( ( servoA0 != -1) && ( valForwardForce0 > 500 ) )
	{
		for (position = 0; position <= backFinDeg; position += 5)
		{
...
/* ServoA1 TILT-UP (UPDOWNFORCE) *************************************************/

  // Left Wing Up 
  if ( ( servoA1 != -1) && ( servoA2 != -1) && ( valUpDownForce3 >= 128 ) )
	{
			ESP32_ISR_Servos.setPosition(servoA1, 180);
			ESP32_ISR_Servos.setPosition(servoA2, 0);      
			// waits 30ms for the servo to reach the position
			delay(30);
      Serial.println("Wings upPos");      
		} 

  if ( ( servoA1 != -1) && ( servoA2 != -1) && ( valUpDownForce3 <= 126 ) )
	{
			ESP32_ISR_Servos.setPosition(servoA1, 0);
 			ESP32_ISR_Servos.setPosition(servoA2, 180);     
			// waits 30ms for the servo to reach the position
			delay(30);
      Serial.println("Wings downPos");

		} 

  if ( ( servoA1 != -1) && ( servoA2 != -1) && ( valUpDownForce3 == 127 ) )
	{
			ESP32_ISR_Servos.setPosition(servoA1, 90);
			ESP32_ISR_Servos.setPosition(servoA2, 90);      
			// waits 30ms for the servo to reach the position
			delay(30);
      Serial.println("Wings middlePos");      
		} 

/* ServoA1 / ServoA2 TURN RIGHT (UPDOWNFORCE) ********************************/

  if ( ( servoA1 != -1) && ( servoA2 != -1) && ( valRightForce1 > 500 ) )
    {
      ESP32_ISR_Servos.setPosition(servoA1, 180);
			ESP32_ISR_Servos.setPosition(servoA2, 0);     
    }
...
/* ServoA1 / ServoA2 TURN LEFT (UPDOWNFORCE) ********************************/

  if ( ( servoA1 != -1) && ( servoA2 != -1) && ( valLeftForce2 > 500 ) )
    {
      ESP32_ISR_Servos.setPosition(servoA1, 0);
			ESP32_ISR_Servos.setPosition(servoA2, 180);     
    }
  ...
  if ( ( servoA1 != -1) && ( servoA2 != -1) && ( valLeftForce2 < 500 ) && ( valRightForce1 < 500 ) )
  {
      ESP32_ISR_Servos.setPosition(servoA1, 90);
			ESP32_ISR_Servos.setPosition(servoA2, 90);   
  }
